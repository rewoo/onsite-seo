...
crawler.crawlPage = function(address) {
  var deferred = q.defer(), isClosed = false, pageResponse;
  ...
  page.open(address, function(status) {
    if (status !== 'success' || pageResponse.status >= 300) {
      console.log('Failed to load '+address+' with open status ' + status + ' and response status ' + pageResponse.status);
      deferred.reject({url: address, err: "Load error", status: 'fail'});
      pageCloseSave();
    } else {
