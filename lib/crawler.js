...
crawler.crawlPage = function(address) {
  ...
  page.onError = function(msg, trace) {
    ...
    deferred.resolve({url: address, err: msg, trace: trace});
    pageCloseSave();
  };
  ...
  page.open(address, function(status) {
    if (status !== 'success') {
      ...
      deferred.resolve({url: address, err: "Load error", status: status});
      pageCloseSave();
    } else {
      if (!page.injectJs("jquery-1.10.1.min.js")) {
        deferred.resolve({url: address, err: 'Could not inject jquery', status: status});
      } else {
