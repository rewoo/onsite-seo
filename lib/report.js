...
report.reportPage = function(title, page, site) {
  ...
  var scoreTemplate = '<p>Score: {{score}}, weighted score: {{weightedScore}}, weight: {{weight}}%</p>\n';
  ...
  cRatings += template.render(reportTemplate, {title: 'Summary', rows: rows});

  // #################################
  // Socials Summary
  // #################################
  summary = socials.getPageSocial(page);
  rows = '';
  for (var i in summary) {
    rows += template.render(rowTemplate, summary[i]);
  }
  cSocials += template.render(reportTemplate, {title: 'Summary', rows: rows});

  var groups = ratings.getGroups(),
      totalWeight = ratings.getWeightSum();
  for (var i in groups) {
    var group = groups[i],
        ...
        weightedScore = 0,
        suggestions = [],
        groupWeight = ratings.getWeightSumOfRatings(fns),
        footer = '',
        header;
    ...
    for (var j in fns) {
      ...
      score += result.score;
      weightedScore += result.score * fns[j].weight;
    ...
    header = template.render(scoreTemplate, {score: score.toFixed(2), weightedScore: weightedScore.toFixed(2), weight: (100 * groupWeight / totalWeight).toFixed(2) });
    if (suggestions.length) {
      footer = template.render(suggestionsTemplate, {count: suggestions.length, suggestionsList: '<li>' + suggestions.join('</li><li>') + '</li>'});
    }
    cRatings += template.render(reportTemplate, {title: 'Group ' + group, rows: rows, header: header, footer: footer});
  }
  return template.renderFile('pageSummary.html', {id: utils.slug(title), url: page.url, ratings: cRatings, socials: cSocials, title: title});
};
